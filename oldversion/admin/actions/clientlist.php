 ...
 //Получаем данные
  ...
  //Если переменная Name передана
    if (isset($_POST["redpost"])) {
      ...
      if (isset($_GET['red'])){
        $sql = mysqli_query($link, "UPDATE `client` SET `fio` = '{$_POST['fio']}',`adres` = '{$_POST['adres']}',`login` = '{$_POST['login']}', `phone` = '{$_POST['phone']}' WHERE `kod_client`={$_GET['red']}");
      }
      ...
      if ($sql) {
				echo"
					...
					<script>
						setTimeout(function() { location.replace('/admin/actions/clientlist') }, 1600);
					</script>";
      } else {
    ...
    //Удаляем, если что
    if (isset($_GET['del'])) {
      $sql = mysqli_query($link, "DELETE FROM `client` WHERE `kod_client` = {$_GET['del']}");
      if ($sql) {
					echo"
					<div class='blockbackdrop'>
					<div class='alert alert-success text-center fixed-bottom shadow-lg p-4 mb-4' style='animation-fill-mode: forwards; animation: show 1s 1;'>
    ...
    if (isset($_GET['red'])) {
		...
		$sql = mysqli_query($link, "SELECT * FROM `client` WHERE `kod_client`={$_GET['red']}");
	...
	<form action='' method='post'>
	 <div class='form-group'>
		<label for='usr'> Код клиента:<br>
			<input disabled class='input form-control' id='kod_client' name='kod_client' type='text' size='50'  value='" , isset($_GET['red']) ? $product['kod_client'] : '' , "'>
		</label>
	  </div>
	  <div class='form-group'>
	  ...
	  <div class='form-group'>
		<label for='usr'>Новый адрес:<br>
			<input required class='input form-control' id='adres' name='adres' type='text' size='50' placeholder='Москва, ул. Краснополянская 20А' value='", isset($_GET['red']) ? $product['adres'] : '' ,"'>
		</label>
	  </div>
	   <div class='form-group'>
	...
	{
     location.replace('/admin/actions/clientlist')
	 }
	</script>
                ...
                <?php if(!empty($arr_users)) { ?>
                    <?php foreach($arr_users as $user) { ?>
                        <tr>
							...
							<td class="text-center"> <a href="?del=<?php echo $user['kod_client'];?>" title="Удалить">
								<input type="image" src="/img/actions/delete.png" style="height:25px;"/></a>
							</td> 
							<td class="text-center"><a href="?red=<?php echo $user['kod_client'];?>" title="Редактировать">
